...
steadyStateFile = open(path + 'results/date_zbar_irr.dat','r')
lines = steadyStateFile.readlines()
steadyStateFile.close()
for line in lines:
    splitLine = line.split()
...
steadyStateFile = open(path + 'results/date_zbar_nirr.dat','r')
lines = steadyStateFile.readlines()
steadyStateFile.close()
for line in lines:
    splitLine = line.split()
...
for n in range(drainages):
    ...
    print('Saving images/D_to_W_sub_{0:03d}.png'.format(n+1))
    plt.savefig('images/D_to_W_sub_{0:03d}.png'.format(n+1))
    plt.show()
    plt.close()
